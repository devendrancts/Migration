...
export class NodeJsNamingConvention implements TargetNamingConvention {
  ...
  fileName(logicalName: string, artifactKind: string): string {
    const kebab = toKebabCase(logicalName);
    return `${kebab}.${artifactKind}.ts`;
  }
...
function splitWords(name: string): string[] {
  // Handle SCREAMING_SNAKE_CASE and kebab-case first
