...
## Corpus Check
- 93 files · ~52,938 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 756 nodes · 1515 edges · 25 communities detected
...
Cohesion: 0.05
Nodes (38): ApiVersioningSkill, AuthSkill, CachingSkill, collectEntries(), collectFromWebConfig(), ConfigSkill, findConfigFiles(), inferProvider() (+30 more)

### Community 1 - "Community 1"
Cohesion: 0.04
Nodes (23): JavaSpringPlatform, NodeJsExpressPlatform, PythonFastApiPlatform, buildPackageDirs(), JavaArchitectureAdapter, toPascalCase(), depToXml(), JavaDependencyManager (+15 more)
...
Cohesion: 0.07
Nodes (35): findAllByType(), findFirstByType(), getAttributeArguments(), getAttributeNames(), getClassBaseTypes(), getModifiers(), walkTree(), findCSharpWasm() (+27 more)
...
Cohesion: 0.07
Nodes (11): dataAnnotationsToValidation(), inferBoundedContext(), nodesByRoles(), MiddlewareSkill, ModelSkill, RoutingSkill, MigrationContext, SkillOrchestrator (+3 more)
...
Cohesion: 0.17
Nodes (6): BackgroundJobsSkill, DataAccessSkill, nodesByRole(), HealthCheckSkill, ServiceSkill, SignalRSkill
...
- **Thin community `Community 9`** (23 nodes): `DddStrategy`, `.getRequiredScaffoldRoles()`, `.modulePath()`, `.resolveAuthPath()`, `.resolveCommandPath()`, `.resolveConfigPath()`, `.resolveControllerPath()`, `.resolveDiContainerPath()`, `.resolveDomainEventPath()`, `.resolveDomainServicePath()`, `.resolveDtoPath()`, `.resolveEntityPath()`, `.resolveEnumPath()`, `.resolveMapperPath()`, `.resolveMiddlewarePath()`, `.resolveQueryPath()`, `.resolveRepositoryImplPath()`, `.resolveRepositoryInterfacePath()`, `.resolveRoutePath()`, `.resolveServicePath()`, `.resolveUseCasePath()`, `.resolveValidationPath()`, `.resolveValueObjectPath()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 10`** (22 nodes): `CleanStrategy`, `.getRequiredScaffoldRoles()`, `.resolveAuthPath()`, `.resolveCommandPath()`, `.resolveConfigPath()`, `.resolveControllerPath()`, `.resolveDiContainerPath()`, `.resolveDomainEventPath()`, `.resolveDomainServicePath()`, `.resolveDtoPath()`, `.resolveEntityPath()`, `.resolveEnumPath()`, `.resolveMapperPath()`, `.resolveMiddlewarePath()`, `.resolveQueryPath()`, `.resolveRepositoryImplPath()`, `.resolveRepositoryInterfacePath()`, `.resolveRoutePath()`, `.resolveServicePath()`, `.resolveUseCasePath()`, `.resolveValidationPath()`, `.resolveValueObjectPath()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 11`** (22 nodes): `MvcStrategy`, `.getRequiredScaffoldRoles()`, `.resolveAuthPath()`, `.resolveCommandPath()`, `.resolveConfigPath()`, `.resolveControllerPath()`, `.resolveDiContainerPath()`, `.resolveDomainEventPath()`, `.resolveDomainServicePath()`, `.resolveDtoPath()`, `.resolveEntityPath()`, `.resolveEnumPath()`, `.resolveMapperPath()`, `.resolveMiddlewarePath()`, `.resolveQueryPath()`, `.resolveRepositoryImplPath()`, `.resolveRepositoryInterfacePath()`, `.resolveRoutePath()`, `.resolveServicePath()`, `.resolveUseCasePath()`, `.resolveValidationPath()`, `.resolveValueObjectPath()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 20`** (10 nodes): `JavaBuildSystem`, `.build()`, `.installDependencies()`, `.parseBuildErrors()`, `.parseTestFailures()`, `.runCommand()`, `.runCoverage()`, `.runLinter()`, `.runSecurityAudit()`, `.runTests()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 21`** (9 nodes): `JavaTypeMapper`, `.mapAsyncReturnType()`, `.mapCollectionType()`, `.mapDictionaryType()`, `.mapNullableType()`, `.mapToOrmType()`, `.mapType()`, `toBoxedType()`, `java-type-mapper.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 22`** (7 nodes): `NodeJsTypeMapper`, `.mapAsyncReturnType()`, `.mapCollectionType()`, `.mapDictionaryType()`, `.mapNullableType()`, `.mapToOrmType()`, `.mapType()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 23`** (7 nodes): `PythonTypeMapper`, `.mapAsyncReturnType()`, `.mapCollectionType()`, `.mapDictionaryType()`, `.mapNullableType()`, `.mapToOrmType()`, `.mapType()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 25`** (3 nodes): `flattenConfig()`, `parseAppSettings()`, `json-config-parser.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `DddStrategy` connect `Community 9` to `Community 1`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `CleanStrategy` connect `Community 10` to `Community 1`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `MvcStrategy` connect `Community 11` to `Community 1`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
